        ...
        <div className='min-h-screen w-full flex items-center justify-center p-4 bg-gray-50'>
            ...
            <div className='bg-white shadow-lg rounded-lg w-full max-w-md p-8 border border-gray-200'>
                <div className='text-center mb-6'>
                    ...
                    <p className='text-gray-600 text-sm'>Create your account to get started
                    </p>
                </div>
                ...
                <div className='mb-4'>
                    <label htmlFor="fullName" className='block text-gray-700 font-medium mb-2 text-sm'>Full Name</label>
                    <input type="text" className='w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#e23744] transition-colors text-sm' placeholder='Enter your full name' onChange={(e)=>setFullName(e.target.value)} value={fullName} required/>
                </div>
                {/* email */}
                ...
                <div className='mb-4'>
                    <label htmlFor="mobile" className='block text-gray-700 font-medium mb-2 text-sm'>Mobile</label>
                    <input type="tel" className='w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#e23744] transition-colors text-sm' placeholder='Enter your mobile number' onChange={(e)=>setMobile(e.target.value)} value={mobile} required/>
                </div>
                {/* password*/}
                ...
                <div className='mb-5'>
                    <label htmlFor="role" className='block text-gray-700 font-medium mb-2 text-sm'>Select Role</label>
                    <div className='flex gap-2'>
                        {["user", "owner", "deliveryBoy"].map((r) => (
                            <button
                                key={r}
                                className='flex-1 border rounded-lg px-3 py-2 text-center font-medium transition-all duration-200 cursor-pointer text-xs'
                                onClick={()=>setRole(r)}
                                style={
                                   role==r?
                                   {background: '#e23744', color:"white", borderColor: '#e23744'}
                                   :{border:`1px solid #e8e8e8`,color: '#1c1c1c', background: 'white'}
                                }>
            ...
            <button className='w-full font-semibold py-3 rounded-lg transition-all duration-200 bg-[#e23744] text-white hover:bg-[#cb202d] cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed' onClick={handleSignUp} disabled={loading}>
...
{googleLoading ? <ClipLoader size={18} color='#e23744'/> : <><FcGoogle size={20}/><span>Sign up with Google</span></>}
            </button>
            <p className='text-center mt-6 cursor-pointer text-gray-600 text-sm' onClick={()=>navigate("/signin")}>Already have an account?  <span className='text-[#e23744] font-semibold hover:text-[#cb202d] transition-colors'>Sign In</span></p>
            </div>
        </div>
    )
}
